...
main_dir = '/global/scratch/hpeter/composites/'
...
save_data_file = '{}data_total_n{}_{}_{}lags.npz'.format(main_dir + 'datasets/', N, variable, len(lags))
...
for i in range(len(lags)):
    ...
    # Save for plotting
    fname = '{}composite_n{}_{}_JJAS_lag{}.nc'.format(main_dir, N, variable, lags[i])
...
for i in range(len(lags)):
    ...
    # Save for plotting
    fname = '{}composite_n{}_{}_JJAS_lag{}_ttest.nc'.format(main_dir, N, variable, lags[i])
    comp_da.to_netcdf(fname)
    log('Data saved in {}.'.format(fname))
