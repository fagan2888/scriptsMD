...
def stratify_data(track_data, PC_dates, PC_vals):
    ''' 
    ...
    '''
    strat_array = np.zeros( (track_data.shape[0], 4) )
    for i in range(track_data.shape[0]):
        lon, lat, year, month, day, hour, intensity = track_data[i, :]
...
def stratify_data_vort(track_data, PC_dates, PC_vals):
    ''' 
    Stratify the Genesis Point data based on EOF strength/phase.
      data:    track data, array
      returns array with each element  [biweekly_phase#, biweekly_strength, weekly_phase#, weekly_strength] for each element of 'data'
    '''
    strat_array = np.zeros( (track_data.shape[0], 2) )
    for i in range(track_data.shape[0]):
        lon, lat, year, month, day, hour, intensity = track_data[i, :]
        ...
        ampW = np.sqrt(eof1**2 + eof2**2)
        angleW = get_angle_deg(eof1, eof2)
        phaseW = get_phase(angleW)

        strat_array[i, :] = [ampW, phaseW]
...
if __name__ == '__main__':
    ...
    #    strat_array = stratify_data(track, PC_dates, PC_vals)
    #    if mode == 'biweekly':
    ...
    strat = MD_set[np.where(np.logical_and( strat_array[:, 0] >= amp_thresh, 
                            ...
                            np.logical_or(strat_array[:, 1] == int(phases[2]),
                                          strat_array[:, 1] == int(phases[3]))))))]
    print('After stratification: {}'.format(strat.shape[0]))
    fname = 'BoB_{}_pts_int{}_{}{}_vort850.npz'.format(points_type, intensities, mode, phases)
    np.savez(fname, strat)
    print('Saved to {}.'.format(fname))
    ##################################################################################
    #### PLOT DATA
    ...
    #    ax = plt.subplot(111)
    #    
    #    ax.set_title('Distribution of {} (vort850) EOFs in JJAS'.format(mode.capitalize()), size=16)
    #    ax.set_xlabel('Phase', size=12)
    #    ax.set_ylabel('# Total Points in Phase', size=12)
    #    
    #    centers = np.arange(1, 9)
    ...
    #    ax.legend(loc='center right')
    #    
    #    fname = 'bars_seasonal_totals_{}_vort850.png'.format(mode)
    #    plt.savefig(fname, dpi=120)
    #
    #f = plt.figure(figsize=(8,5))
    #ax = plt.subplot(111)
    #
    #ax.set_title('Distribution of MD {} points\nin BoB JJAS: {} (vort850) EOFs'.format(points_type.capitalize(), mode.capitalize()), size=16)
    #ax.set_xlabel('Phase', size=12)
    #if normalize:
    #    ax.set_ylabel('# Track Points / # Total Points in Phase', size=12)
    #else:
    ...
    #
    #if normalize:
    #    fname = 'bars_{}_pts_fraction_{}_vort850.png'.format(points_type, mode)
    #else:
    #    fname = 'bars_{}_pts_totals_{}_vort850.png'.format(points_type, mode)
    #plt.savefig(fname, dpi=120)
    #
    #plt.show()
